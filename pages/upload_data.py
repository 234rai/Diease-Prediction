...
tab1, tab2 = st.tabs(["Upload Dataset for Model Training", "More"])
...
with tab1:
    ...
    # File uploader for ZIP containing a CSV
    uploaded_file = st.file_uploader(
        "Upload a ZIP file containing a CSV", type=["zip"], accept_multiple_files=False,
        help="Upload a ZIP file containing a single CSV file with disease and symptom data", key="tab1_file_uploader"
    )
    ...
    # Upload and process the ZIP file
        ...
        if uploaded_file is not None and existing_model and dataset_version:
            if abs((train_ratio + val_ratio + test_ratio) - 1.0) > 1e-5:
                ...
                with tempfile.TemporaryDirectory() as temp_dir:
                    ...
                    if len(csv_files) == 0:
                        st.error("No CSV file found in the uploaded ZIP.")
                    elif len(csv_files) > 1:
                        st.error("Multiple CSV files found in the ZIP. Please include only one CSV file.")
                    else:
                        csv_path = os.path.join(temp_dir, csv_files[0])
                        # Split and save the CSV data
                        split_and_save_csv(csv_path, version_folder, train_ratio, val_ratio, test_ratio)
        else:
            st.error("Please upload a ZIP file, select or add a model, and provide a version name.")
